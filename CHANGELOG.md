...
- Select fields now support per-option colors: assign a hex color to each option via the board setup Fields tab; matching values render as colored pill chips on the card face; the `colors` config key is also available for hand-editing
