...
describe('version key', () => {
	...
	it('parses an explicit version: 3 and is writable', () => {
	...
	it('marks a version 4 board as readonly', () => {
		const block = MINIMAL_BLOCK.replace('title: My Board', 'version: 4\ntitle: My Board');
		const result = parseBlock(block);
		expect(result.ok).toBe(true);
		if (!result.ok) return;
		expect(result.readonly).toBe(true);
		if (!result.readonly) return;
		expect(result.readonlyReason).toMatch(/version 4/);
	});

	it('serializer emits version: 3', () => {
		const result = parseBlock(MINIMAL_BLOCK);
		if (!result.ok) return;
		const serialized = serializeBoard(result.board);
		expect(serialized).toMatch(/^version: 3/m);
	});
});
